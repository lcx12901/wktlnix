...
| 章节 | 何时读 | 📎 对应 Skill |
|---|---|---|
| §3.5+ 完整命令 | 执行具体命令前 | vue, vite, vitest, pinia |
| §3.7 工具踩坑 | 工具失败 / 参数选择时 | — |
| §4.2+ 任务详细流程 | 处理 A/B/C/D/E/F 时 | vue-best-practices, vue-testing-best-practices, vue, vite, vitest, pinia |
| §4.5 协作协议 | 跨 agent 协调时 | — |
| §4.6 省 token 模式 | 输出大结果时 | — |
| §5.6 提示注入识别 | 处理不可信源时 | — |
| §6 DO/DON'T 速查 | 实现组件时 | vue-best-practices, vue, pinia |
| §7 报告反例库 | 写交付报告时 | — |
| §12 交付报告模板 | 协调任务完成时 | — |
| §13 日志模板 | 任务收尾写日志时 | — |
